...
const User3 = () => {
    ...
    const getAllUsers=()=>{
        axios.get("http://localhost:3009/User3").then((response)=>{
            console.log(response.data)
            setdata(response.data)
        })
    }
    const addUser = () => {
            axios.post("http://localhost:3009/User3/",persions).then((res)=>{
                getAllUsers()
                clearForm()
            })
    }
    ...
    const delUser=(val)=>{
         axios.delete("http://localhost:3009/User3/"+val.id).then((res)=>{
            getAllUsers()
         })
    }
    const updateUser=()=>{
        axios.put("http://localhost:3009/User3/"+persions.id,persions).then((res)=>{
            setisedit(false)
            getAllUsers()
            clearForm()
